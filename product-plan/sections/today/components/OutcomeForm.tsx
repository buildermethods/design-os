...
const nextActionIcons: Record<NextAction, typeof Send> = {
  'send_demo_link': Send,
...
export function OutcomeForm({
  ...
  const needsFollowUp = nextAction && ['send_demo_link', 'send_payment_link', 'schedule_follow_up'].includes(nextAction)

  // Filter next actions based on connection status
  const filteredNextActions = nextActionOptions.filter((opt) => {
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {callResultOptions.map((option) => {
            const Icon = callResultIcons[option.id]
            ...
            return (
              <button
                key={option.id}
                onClick={() => setCallResult(option.id)}
                className={`
              ...
              <div key={category.category}>
                ...
                <div className="flex flex-wrap gap-2">
                  {category.options.map((option) => (
                    <button
                      key={option.id}
                      onClick={() => setConversationOutcome(option.id)}
                      className={`
      ...
      {isConnected && (
        <div className="animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {filteredNextActions.map((option) => {
              const Icon = nextActionIcons[option.id]
              const isCloseAction = ['close_won', 'close_lost'].includes(option.id)
